...
static void json_dumpinfo(struct bfdev_json_node *parent, unsigned int depth)
{
    ...
    bfdev_list_for_each_entry(child, &parent->child, sibling) {
        ...
        printf("%s: ", child->name);
        if (bfdev_json_test_number(child))
...
int main(void)
{
    struct bfdev_json_node *jnode;
    int length, retval;
    char *buff;

    retval = bfdev_json_decode(NULL, json_test, &jnode);
    if (retval)
        return retval;
    ...
    printf("json encode:\n");
    length = bfdev_json_encode(jnode, NULL, 0);

    buff = malloc(length);
    if (!buff)
        return 1;

    length = bfdev_json_encode(jnode, buff, length);
    fwrite(buff, length, 1, stdout);
    free(buff);

    bfdev_json_release(NULL, jnode);
    return retval;
}
