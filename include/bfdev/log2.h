...
bfdev_ilog2_dynamic(unsigned long val)
{
    return bfdev_fls(val) - 1;
}
 ...
 * @val: value to log2.
 */
#define bfdev_ilog2(val) (              \
    __builtin_constant_p(val) ?         \
    ((val) < 2 ? 0 :                    \
     63 - __builtin_clzll(val)) :       \
    (sizeof(val) <= 4) ?                \
    bfdev_ilog2_dynamic(val) :          \
    bfdev_ilog2_64_dynamic(val)         \
)
