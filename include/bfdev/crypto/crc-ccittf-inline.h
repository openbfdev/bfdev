...
#ifndef _BFDEV_CRYPTO_CRC_CCITTF_INLINE_H_
#define _BFDEV_CRYPTO_CRC_CCITTF_INLINE_H_
...
static inline uint16_t
bfdev_crc_ccittf_inline(const uint8_t *src, size_t len, uint16_t crc)
{
    for (; len >= 8; len -= 8) {
        crc = bfdev_crc_ccittf_byte(crc, *src++);
        crc = bfdev_crc_ccittf_byte(crc, *src++);
        crc = bfdev_crc_ccittf_byte(crc, *src++);
        crc = bfdev_crc_ccittf_byte(crc, *src++);
        crc = bfdev_crc_ccittf_byte(crc, *src++);
...
#endif /* _BFDEV_CRYPTO_CRC_CCITTF_INLINE_H_ */
