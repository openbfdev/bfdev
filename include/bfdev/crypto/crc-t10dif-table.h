...
#ifndef _BFDEV_CRYPTO_CRC_T10DIF_TABLE_H_
#define _BFDEV_CRYPTO_CRC_T10DIF_TABLE_H_
...
static const uint16_t bfdev_crc_t10dif_table[256] = {
...
bfdev_crc_t10dif_byte(uint16_t crc, const uint8_t byte)
{
    return (crc << 8) ^ bfdev_crc_t10dif_table[((crc >> 8) ^ byte) & 0xff];
}

static inline uint16_t
bfdev_crc_t10dif_inline(const uint8_t *src, size_t len, uint16_t crc)
{
    for (; len >= 8; len -= 8) {
        crc = bfdev_crc_t10dif_byte(crc, *src++);
        crc = bfdev_crc_t10dif_byte(crc, *src++);
        crc = bfdev_crc_t10dif_byte(crc, *src++);
        crc = bfdev_crc_t10dif_byte(crc, *src++);
        crc = bfdev_crc_t10dif_byte(crc, *src++);
...
#endif /* _BFDEV_CRYPTO_CRC_T10DIF_TABLE_H_ */
