...
#ifndef _BFDEV_CRYPTO_CRC_ITUT_TABLE_H_
#define _BFDEV_CRYPTO_CRC_ITUT_TABLE_H_
...
static const uint16_t bfdev_crc_itut_table[256] = {
...
bfdev_crc_itut_byte(uint16_t crc, const uint8_t data)
{
    unsigned int index = ((crc >> 8) ^ data) & 0xff;
    return bfdev_crc_itut_table[index] ^ (crc << 8);
}

static inline uint16_t
bfdev_crc_itut_inline(const uint8_t *src, size_t len, uint16_t crc)
{
    for (; len >= 8; len -= 8) {
        crc = bfdev_crc_itut_byte(crc, *src++);
        crc = bfdev_crc_itut_byte(crc, *src++);
        crc = bfdev_crc_itut_byte(crc, *src++);
        crc = bfdev_crc_itut_byte(crc, *src++);
        crc = bfdev_crc_itut_byte(crc, *src++);
...
#endif /* _BFDEV_CRYPTO_CRC_ITUT_TABLE_H_ */
