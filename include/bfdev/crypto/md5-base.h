...
bfdev_md5_base_update(bfdev_md5_ctx_t *ctx, const void *data,
                      bfdev_size_t size, bfdev_md5_bfn_t func)
{
    unsigned long avail;
    ...
    if (avail > size) {
        bfdev_memcpy((void *)ctx->block + (sizeof(ctx->block) - avail),
                      data, size);
        return;
    }

    bfdev_memcpy((char *)ctx->block + (sizeof(ctx->block) - avail),
                  data, avail);
    func(ctx);
    data += avail;
    size -= avail;
    ...
    while (size >= sizeof(ctx->block)) {
        bfdev_memcpy(ctx->block, data, sizeof(ctx->block));
        func(ctx);
        data += sizeof(ctx->block);
        size -= sizeof(ctx->block);
    }

    bfdev_memcpy(ctx->block, data, size);
}
...
bfdev_md5_base_finalize(bfdev_md5_ctx_t *ctx, bfdev_md5_bfn_t func)
{
    ...
    *ptr++ = 0x80;
    ...
    padding = 56 - (offset + 1);
    if (padding < 0) {
        bfdev_memset(ptr, 0, padding + BFDEV_BYTES_PER_U64);
        func(ctx);
        ptr = (char *)ctx->block;
        padding = 56;
    }

    bfdev_memset(ptr, 0, padding);
    ctx->block[14] = ctx->count << 3;
    ctx->block[15] = ctx->count >> 29;
    bfdev_le32_to_cpu_array(ctx->block, (bfdev_le32 *)ctx->block,
        (sizeof(ctx->block) - BFDEV_BYTES_PER_U64) / BFDEV_BYTES_PER_U32);
    func(ctx);
}
...
bfdev_md5_base_final(bfdev_md5_ctx_t *ctx, void *buff)
{
    ...
    bfdev_memcpy(buff, ctx->digest, sizeof(ctx->digest));
}

static inline void
bfdev_md5_digest_init(bfdev_u32 *digest)
{
